...
def Rotate():
    ...
    while Rotation_Status == True:
        tp = time.time() - start
        #new eqs7/3/23:
        Bx = A * ((-np.sin(alpha) * np.sin(omega*tp)) + (-np.cos(alpha) * np.cos(gamma)  * np.cos(omega*tp))) 
        By = A * ((np.cos(alpha) * np.sin(omega*tp)) + (-np.sin(alpha) * np.cos(gamma) *  np.cos(omega*tp))) 
