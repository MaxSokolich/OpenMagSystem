...
def Handle_Zero():
    '''
    need to add the code that will turn off all signals to ALL electromagnetics
    '''
...
def Handle_Xbox():
    ...
    def Right_Trigger():
        ...
        Z_Strength_Entry.insert(0, str(round(joy.rightTrigger(),2))) #print 0 -> +1 as positive Z
    ...
    def Right_Joystick():#i, t_list, Bx_List, By_List, Bz_List):
        ...
        #taking magntiude of x,y joystick position and multipling it by 12. so rolling speed varys with joystick yaw
        mapped_frequency = np.sqrt(joy.rightX()**2 + joy.rightY()**2) * 4  
        omega = 2*np.pi* float(Rot_Freq_Entry.get()) #Rot_Freq_Entry.get() #angular velocity of rotating field defined from input from Rotating Frequency Entry
